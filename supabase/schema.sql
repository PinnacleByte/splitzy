...
-- by accept_invite(). "My friends" == everyone I have a connections row with.
...
-- Invite links. code is a short random slug generated by the app
-- (see app/friends/actions.ts createInvite). group_id is null for a
-- friend-only invite, or set to join the invitee straight into a group.
...
-- connections: readable by either side; writes only via accept_invite().
...
-- invites: only the creator can see/manage their own invite rows directly.
-- The public /invite/[code] preview and the accept flow go through the
-- SECURITY DEFINER functions below instead, so anon never reads this table.
...
-- =========================================================================
-- Invite functions (SECURITY DEFINER — bypass RLS deliberately, scoped
-- narrowly to what each one is allowed to read/write)
-- =========================================================================
...
begin
  ...
  insert into connections (user_id, friend_id) values (v_invite.created_by, v_uid)
    on conflict do nothing;
  insert into connections (user_id, friend_id) values (v_uid, v_invite.created_by)
    on conflict do nothing;

  if v_invite.group_id is not null then
    insert into group_members (group_id, person_id) values (v_invite.group_id, v_uid)
      on conflict do nothing;
  end if;

  update invites set accepted_by = v_uid, accepted_at = now() where id = v_invite.id;

  return jsonb_build_object('group_id', v_invite.group_id, 'inviter_id', v_invite.created_by);
end;
$$;
